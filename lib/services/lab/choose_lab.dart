      ...
      builder:(context, state) {
        return Scaffold(
          ...
          body: ListView.separated(
                itemBuilder: (context, index) => BlocBuilder<AppBloc, AppState>(
                  builder: (context, state) {
                   return Container(
                     ...
                     child: InkWell(onTap: (){
                       navigateTo(context, pcr(labName: AppBloc.get(context).LabList[index].name));
                     },
                       child: Padding(
                           padding: const EdgeInsets.only(right:10.0,left:10,top: 10,bottom: 10),
                           child: Row(
                            children: [
                              CircleAvatar(backgroundImage: NetworkImage(AppBloc.get(context).LabList[index].image),
                              radius: 55,),
                              SizedBox(
                                width: 20,
                              ),
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(AppBloc.get(context).LabList[index].name,
                                    ...
                                    SizedBox(height: 10,),
                                    Text(AppBloc.get(context).LabList[index].email,
                ...
                itemCount: AppBloc.get(context).LabList.length,
              ),
        );
      }
    );
